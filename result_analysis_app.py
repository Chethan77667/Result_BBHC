...
def main():
    ...
    if 'selected_usn' not in st.session_state:
        st.session_state.selected_usn = None
            ...
            if output_filename:
                ...
                if final_df is not None:
                    ...
                    if st.session_state.page == 'main':
                        ...
                        cols = st.columns([1, 2, 2, 2, 2, 2, 1])  # Adjust as needed for your columns
                        ...
                        if st.button("← Back to Results"):
                            ...
                            st.experimental_rerun()
                        # Get student data
                        student_result = final_df[final_df["USN"] == st.session_state.selected_usn]
                ...
                for file_info in search_results:
                    with st.expander(f"📄 {file_info['filename']}"):
                        ...
                        with col2:
                            ...
                            # Print option (opens in new tab)
                            if st.button("🖨️ Print", key=f"print_{file_info['filename']}"):
                                st.markdown(f"<script>window.open('{file_info['file_path']}', '_blank');</script>", unsafe_allow_html=True)
